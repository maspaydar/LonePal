...
import { Heart, Mail, CheckCircle, Loader2, Eye, EyeOff, ArrowLeft } from "lucide-react";
...
export default function FamilySignupPage() {
  ...
  const signupMutation = useMutation({
    mutationFn: async (data: FamilySignupForm) => {
  ...
  function onSubmit(data: FamilySignupForm) {
    ...
    <div className="min-h-screen bg-muted/30 flex items-center justify-center p-4">
      <div className="w-full max-w-lg space-y-6">
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="text-2xl font-bold">Sign up for your loved one</h1>
          <p className="text-muted-foreground text-sm mt-1">
            Daily check-ins and instant alerts — 30-day free trial, no credit card required
          </p>
        </div>

        <Card>
          <CardHeader className="pb-4">
            <CardTitle className="text-base">Tell us about you and your loved one</CardTitle>
            <CardDescription>We'll set up companionship and safety monitoring for the person you care about.</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Your loved one's name</FormLabel>
                        <FormControl>
                          <Input
                            placeholder="Ruth"
                            data-testid="input-loved-one-name"
                            {...field}
                          />
                        </FormControl>
                        <FormMessage />
                ...
                <Button
                  ...
                  data-testid="button-family-signup"
                >
                ...
                <p className="text-center text-sm text-muted-foreground">
                  Run a senior living facility?{" "}
                  <button
                    type="button"
                    onClick={() => setLocation("/register")}
                    className="text-primary underline-offset-4 hover:underline"
                    data-testid="link-go-to-facility-register"
                  >
        ...
        <p className="text-center text-xs text-muted-foreground">
          By signing up, you agree to our terms of service. Your 30-day trial starts upon email verification.
        </p>
      </div>
    </div>
  );
